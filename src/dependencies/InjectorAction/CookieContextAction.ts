...
export class CookieContextAction extends AbstractAction {
  ...
  public run(instance: GenericControllerInstance, method: string): InjectorFunctionParameter {
    if (!Reflect.hasMetadata(REFLECT_METADATA.CONTEXT_COOKIE, instance, method)) {
      return
    }

    const metadata = Reflect.getMetadata(
      REFLECT_METADATA.CONTEXT_COOKIE,
      instance,
      method,
    ) as number
