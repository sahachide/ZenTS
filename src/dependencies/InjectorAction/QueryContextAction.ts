...
export class QueryContextAction extends AbstractAction {
  ...
  public run(instance: GenericControllerInstance, method: string): InjectorFunctionParameter {
    if (!Reflect.hasMetadata(REFLECT_METADATA.CONTEXT_QUERY, instance, method)) {
      return
    }

    const metadata = Reflect.getMetadata(REFLECT_METADATA.CONTEXT_QUERY, instance, method) as number
