...
export class ParamsContextAction extends AbstractAction {
  ...
  public run(instance: GenericControllerInstance, method: string): InjectorFunctionParameter {
    if (!Reflect.hasMetadata(REFLECT_METADATA.CONTEXT_PARAMS, instance, method)) {
      return
    }

    const metadata = Reflect.getMetadata(
      REFLECT_METADATA.CONTEXT_PARAMS,
      instance,
      method,
    ) as number
